...
public interface StatsRepository extends JpaRepository<Hit, Integer> {
    ...
    @Query("select new ru.practicum.model.StatsDto(h.app, h.uri, count(distinct h.ip) as hits) " +
            "from Hit as h " +
    ...
    @Query("select new ru.practicum.model.StatsDto(h.app, h.uri, count(h.ip) as hits) " +
            "from Hit as h " +
            "where h.timestamp between ?1 and ?2 " +
            "group by h.app, h.uri " +
